    ...
    @if(auth()->user()->is_admin || auth()->user()->id == $post->user->id)
        ...
        <div class="flex justify-center">
                <div class ="block w-1/2 p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    ...
                    @endif
                    <form method="POST" action="{{ route('posts.edit.store', ['id' => $post->id]) }}"  enctype="multipart/form-data">
                        @csrf
                        ...
                        <input class="block w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 cursor-pointer dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" name="image" type="file" value = "{{ $post->image }}">
                        ...
                        <div class="flex justify-between">
                            ...
                            <button type="submit" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Edit post</button>
                        </div>
                    </form>
            </div>
            </div>
    @else        
        <h1 class="flex justify-center b-4 text-4xl font-extrabold tracking-tight leading-none text-gray-900 md:text-5xl lg:text-6xl dark:text-white">You can not edit this <span class="underline underline-offset-3 decoration-8 decoration-red-400 dark:decoration-red-600"> post</span></h1>
